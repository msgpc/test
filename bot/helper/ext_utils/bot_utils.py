    ...
    with download_dict_lock:
        ...
        for index, download in enumerate(list(download_dict.values())[COUNT:], start=1):
            ...
            if download.status() not in [
                ...
                if reply_to:
                    msg += f"\n<b>𝗥𝗲𝗾 𝗕𝘆:</b> <a href='tg://user?id={download.message.from_user.id}'>{download.message.from_user.first_name}</a>(<code>{download.message.from_user.id}</code>)"
                else:
                    msg += f"\n<b>𝗥𝗲𝗾 𝗕𝘆:</b> <a href='tg://user?id={download.message.from_user.id}'>{download.message.from_user.first_name}</a>(<code>{download.message.from_user.id}</code>)"
                try:
                    msg += f"\n<i>Aria2📶</i></b> | <b>𝗦𝗲𝗲𝗱𝗲𝗿𝘀:</b> {download.aria_download().num_seeders}" \
                           f" | <b>Peers:</b> {download.aria_download().connections}"
                except:
                    pass
                try: 
                    msg += f"\n<b><i>qbit🦠</i></b> | <b>𝗦𝗲𝗲𝗱𝗲𝗿𝘀:</b> {download.torrent_info().num_seeds}" \
